...
// getClient uses a Context and Config to retrieve a Token then generate client
// It returns the generated Client
func getClient(scope string) *http.Client {
	ctx := context.Background()

	// Read client_secret.json file, this can be obtained from the credentials of Google Cloud
	b, err := ioutil.ReadFile("client_secret.json")
	if err != nil {
		log.Fatalf("Unable to read client secret fiel: %v", err)
	}
